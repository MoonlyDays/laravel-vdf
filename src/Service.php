...
class Service
{
    ...
    public function decode(string $text): Fluent
    {
        ...
        for ($i = 0; $i < $j; $i++) {
            ...
            if ($expect_bracket) {
                throw new VDFException("invalid syntax, expected a '}' on line ".($i + 1), E_USER_NOTICE);
            }
            ...
            while (true) {
                ...
                if (!$m) {
                    throw new VDFException("invalid syntax on line ".($i + 1), E_USER_NOTICE);
                }
        ...
        if (count($stack) !== 1) {
            throw new VDFException("Open parentheses somewhere", E_USER_NOTICE);
        }
